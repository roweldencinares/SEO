...
const routeEntity = nexusify(
    async (entity) => {
        ...
        return {
            ...
            suggestedUrl: `https://${suggestedCluster.subdomain}.example.com/${entity.id}`,
                ...
                ${crossLinks.map(link => `
                    <li style="margin-bottom: 10px;">
                        <a href="${link.url}" style="
                            ...
                            font-weight: 600;
                        ">${link.anchor}</a>
                        <span style="color: #718096; font-size: 14px;"> • ${link.toSubdomain}.example.com</span>
                    </li>
                `).join('')}
            </ul>
        </div>
...
const generateBreadcrumbs = nexusify(
    async (entityId) => {
        ...
        const breadcrumbs = [
            ...
            {
                name: cluster.name,
                url: `https://${cluster.subdomain}.example.com`
            },
            {
                name: entity.name,
                url: entity.url
